...
class P3ProfitEngineTest(unittest.TestCase):
    ...
    def test_quantity_scales_purchase_cost_sale_price_and_fees(self) -> None:
        estimate = self.engine.evaluate(
            ProfitInput.from_dict(
                {
                    "id": "profit-quantity",
                    "p4_decision": "same",
        ...
        self.assertEqual(2, estimate.quantity)
        self.assertEqual(480, estimate.fees.platform_fee_yen)
        self.assertEqual(200, estimate.fees.other_cost_yen)
        self.assertEqual(1920, estimate.net_profit_yen)
        self.assertEqual(2500, estimate.break_even_price_yen - estimate.fees.platform_fee_yen - estimate.fees.fulfillment_fee_yen - estimate.fees.other_cost_yen)

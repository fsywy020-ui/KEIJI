...
from keiji.p4_identity import MarketListing, ProductIdentityEngine, SourceOffer, extract_product_attributes
...
class VariantConditionTitleExplainTest(unittest.TestCase):
    ...
    def test_extracts_core_attributes_from_title(self) -> None:
        attributes = extract_product_attributes("新品 sony wh1000xm4 black 256gb 2個セット jan 4548736112100")
        self.assertEqual("4548736112100", attributes.jan)
        self.assertEqual("wh1000xm4", attributes.model)
        self.assertEqual("256gb", attributes.capacity)
        self.assertEqual("black", attributes.color)
        self.assertEqual("2", attributes.set_count)
        self.assertEqual("new", attributes.condition)

    def test_engine_uses_title_extracted_jan_when_structured_fields_are_missing(self) -> None:
        decision = self.engine.evaluate(
            SourceOffer(
                id="src-title-jan",
                title="SONY WH1000XM4 Black JAN 4548736112100",
                brand="SONY",
                condition="new",
            ...
            MarketListing(
                ...
                title="ソニー WH1000XM4 ブラック 4548736112100",
                brand="ソニー",
                condition="new",
            ),
        )
    ...
    def test_title_extracted_unsafe_condition_overrides_default_new(self) -> None:
        decision = self.engine.evaluate(
            SourceOffer(
                id="src-title-used",
                title="中古 SONY WH1000XM4 Black JAN 4548736112100",
                brand="SONY",
                condition="new",
                purchase_price_yen=3000,
            ),
    ...
    def test_title_extracted_set_count_conflict_requires_review(self) -> None:
        decision = self.engine.evaluate(
            SourceOffer(
                id="src-set",
                title="SONY WH1000XM4 Black 2個セット JAN 4548736112100",
                brand="SONY",
                condition="new",
                purchase_price_yen=3000,
            ),
            MarketListing(
                id="amz-set",
                marketplace="amazon_jp",
                title="ソニー WH1000XM4 ブラック 1個入 4548736112100",
                brand="ソニー",
                condition="new",
            ),
        )
    ...
    def test_extracts_edition_and_domestic_import_variants(self) -> None:
        variants = extract_variants("nintendo switch oled 国内正規")
        self.assertEqual("oled", variants["edition"])

...
Mitaさん判断により、Yahoo!デベロッパー登録用公開ページの表向き名称を以下に変更した。
...
GitHub Pages buildは `built`。
通常URLはCDNキャッシュで一時的に旧表示が残る可能性があるため、確認時は `?v=8284156` 付きURLで以下を確認した。

- 3ページとも `輝煌堂 物販API検証` 表示あり。
- 3ページとも `KEIJI 物販API検証` 表示なし。

補足:

- URL内の `/KEIJI/` はGitHubリポジトリ名由来。
- Mitaさん確認により、URL内の `/KEIJI/` は現時点では許容。
- URLからもKEIJIを消す場合は、別repo公開または独自ドメインが必要。

関連コミット:

- `8284156` Yahoo登録ページ名を輝煌堂に変更する

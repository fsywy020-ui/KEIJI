...
Builds an auditable cost breakdown from local/manual purchase input and estimated fees:
...
- Fulfillment fee.
- Storage fee.
- Conservative buffer/reserve cost.

Initial MVP treats prices as tax-inclusive for purchase-decision purposes and does not provide accounting or tax advice.
...
```text
P4 gate -> price selection -> fee estimate -> cost breakdown

...
Estimates Amazon fees with configurable category fee rates and default fee fallback. The current MVP combines platform fee, configured fulfillment fee, storage fee, and other/buffer cost into a deterministic local fee breakdown.

### `shipping_estimator.py`

Initial MVP accepts inbound shipping as local/manual input and uses configured fulfillment/other buffer costs in `fee_estimator.py`. A dedicated estimator remains a future extension once local assumptions need more granularity.
...
Initial MVP applies a deterministic reason-count penalty in `engine.py`; a dedicated module remains a future extension. Conservative penalties cover:

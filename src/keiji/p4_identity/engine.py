...
class ProductIdentityEngine:
    ...
    def evaluate(self, source_offer: SourceOffer, market_listing: MarketListing) -> IdentityDecision:
        source_attributes = extract_product_attributes(
        ...
        listing_attributes = extract_product_attributes(
        ...
        source = normalize_product_text(
            title=source_offer.title,
            brand=source_offer.brand,
            model=source_attributes.model or source_offer.model,
            jan=source_attributes.jan or source_offer.jan,
            asin=source_attributes.asin or source_offer.asin,
            condition=_effective_condition(source_offer.condition, source_attributes),
            rules=self.rules,
        )
        listing = normalize_product_text(
            title=market_listing.title,
            brand=market_listing.brand,
            model=listing_attributes.model or market_listing.model,
            jan=listing_attributes.jan or market_listing.jan,
            asin=listing_attributes.asin or market_listing.asin,
            condition=_effective_condition(market_listing.condition, listing_attributes),
            rules=self.rules,
        )
...
def _effective_condition(explicit_condition: str, attributes: ProductAttributes) -> str:
    """Prefer unsafe condition evidence extracted from text over default values."""

    if attributes.condition in {"junk", "used", "open_box"}:
        return attributes.condition
    return explicit_condition or attributes.condition or "new"

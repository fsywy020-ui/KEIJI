...
def extract_variants(text: str) -> dict[str, str]:
    ...
    return extract_product_attributes(text).variants()
...
def match_variants(source: NormalizedProductText, listing: NormalizedProductText) -> VariantMatchResult:
    ...
    source_variants = source.extracted.variants() or extract_variants(source.title)
    listing_variants = listing.extracted.variants() or extract_variants(listing.title)

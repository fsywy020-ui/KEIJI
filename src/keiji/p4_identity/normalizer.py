...
def normalize_product_text(
    *,
    ...
    normalized_model = normalize_identifier(model, rules)
    normalized_jan = normalize_identifier(jan, rules)
    normalized_condition = normalize_text(condition, rules) or ""
    attributes = extract_product_attributes(
        normalized_title,
        explicit_jan=normalized_jan,
        explicit_model=normalized_model,
        explicit_condition=normalized_condition,
        rules=rules,
    )
    return NormalizedProductText(
        title=normalized_title,
        brand=normalize_brand(brand, rules),
        model=normalized_model or attributes.model,
        jan=normalized_jan or attributes.jan,
        asin=normalize_identifier(asin, rules),
        condition=attributes.condition or normalized_condition,

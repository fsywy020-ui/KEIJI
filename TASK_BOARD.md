...
| Item | Status | Notes |
|---|---|---|
| Manus handoff packet schema | 完了 | `src/keiji/manus_handoff/models.py` に local-only packet を定義。 |
| P7→P8 handoff builder | 完了 | P7 review packet から P8 handoff packet を生成。 |
| Local export JSON/CSV/Markdown | 完了 | `p8_manus_handoff_packets.*` を local 出力。 |
| Manus allowed / forbidden actions policy | 完了 | `docs/manus_handoff_policy.md` と packet policy constants に整理。 |
| Human checklist | 完了 | `docs/manus_human_checklist.md` に非エンジニア向け確認項目を整理。 |
| Blocked actions policy | 完了 | `docs/blocked_actions_policy.md` と security tests を追加。 |
| P7→P8 integration test | 完了 | P4/P3/P5/P6/P7 から P8 handoff export まで検証。 |
| Smoke workflow P8 output | 完了 | `scripts/local_smoke.py` が P8 handoff files を出力。 |

## 次工程候補

| Phase | Status | Goal | Safety boundary |
|---|---|---|---|
| P8-2 Manus実行プロンプト設計 | 未着手 | local handoff packet を読み、人間確認を支援する prompt template を作る | Manus に login/cart/checkout/purchase/payment/listing/browser automation/scraping/live API をさせない |
| P9 販売・出品前準備 | 未着手 | 出品前チェック、カテゴリ・状態・法務/古物商記録の local checklist を作る | 出品実行や外部 marketplace 操作はしない |
| External API検討 | Owner判断待ち | 対象 API と目的を明示承認するか判断 | 明示承認なしに adapter を作らない |

---

## PR #6 Scope Cleanup: P8 Manus Handoff Only

| Item | Status | Notes |
|---|---|---|
| Branch scope review | 完了 | PR #6 should be P8-only on top of main where PR #5 is already merged. |
| P5〜P7 duplicate scope | 除外対象 | Treat P5〜P7 as already-main PR #5 content, not PR #6 review scope. |
| P8 handoff packet | 完了 | `src/keiji/manus_handoff/` remains PR #6 scope. |
| P8 docs | 完了 | Manus handoff policy, human checklist, and blocked actions policy remain PR #6 scope. |
| P8 tests | 完了 | Unit, integration, and blocked-action audit tests remain PR #6 scope. |
| Mergeability | Owner/GitHub確認待ち | Local container has no remote `origin` or `main`; GitHub must confirm `mergeable: true`. |

次工程は P8-2 Manus実行プロンプト設計。ただし、購入、決済、出品、checkout、login、cart 操作、browser automation、scraping、live external API は引き続き禁止。

---

## Replacement PR for Diverged PR #6

| Item | Status | Notes |
|---|---|---|
| Old PR #6 | マージ不要 | Diverged and contains duplicate P5〜P7 diffs. |
| Replacement branch | 作成 | `prepare-p8-manus-handoff-safety-contract` created locally. |
| Replacement scope | P8のみ | Manus handoff safety contract, docs, tests, smoke P8 output, P8 docs/status updates. |
| P5〜P7 duplicate diffs | 含めない | PR #5 already merged them to main. |
| Local tests | PASS | pytest / unittest / local smoke passed. |
| Merge readiness | READY locally | GitHub must confirm latest-main base and P8-only diff. |

---

## Replacement PR v2: Prepare P8 Manus Handoff Safety Contract

| Item | Status | Notes |
|---|---|---|
| Old PR #6 | マージ不要 | Still mergeable false; do not continue or merge. |
| Replacement branch v2 | 作成 | `prepare-p8-manus-handoff-safety-contract-v2`. |
| Intended scope | P8のみ | Manus handoff safety contract only. |
| Excluded scope | P5〜P7 / P4 | Must not appear as new diff because PR #5 already merged them. |
| Local tests | PASS | pytest / unittest / local smoke passed on replacement branch v2. |
| Merge readiness | READY locally | GitHub must confirm latest-main base and P8-only diff. |
